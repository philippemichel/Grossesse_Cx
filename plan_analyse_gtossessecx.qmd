...
L'analyse du suivi par le médecin généraliste sera uniquement descriptive.  Si le nombre de femme ayant choisi le MG pour leur suivi de grossesse est suffisant( > 30) une comparaison entre ce groupe & les autres participantes sera réalisé par des analyses par des tests de  $\chi^2$ en reprenant les items des trois parties.


Un modèle de régression logistique sera employé. On utilisera si possible modèle binomial (après vérification des hypothèses de normalité et d’homoscédasticité des résidus) sinon un modèle pseudo-binomial sera employé. Ce modèle de régression permettra d’explorer les facteurs associés à la réponse en utilisant les critères ayant une la réponse significative à p=0,2 lors de l'étape précédente. Une recherche du meilleur modèle sera réalisée par un step-by-step descendant évaluée par le critère d’information d’Akaike. Cette étape sera réalisée après imputation des données manquantes mais le résultat final sera calculé sur les données vraies.
...
Un dépôt GIT sera utilisé qui ne comprendra que le code & non les données ou résultats. Au besoin un faux tableau de données sera présent pour permettre des tests.
